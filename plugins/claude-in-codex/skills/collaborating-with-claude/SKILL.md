...
The same payloads are also readable as MCP resources, for clients that browse rather than call:
`claude-in-codex://models` (identical to `claude_models`) and `claude-in-codex://capabilities`.
`claude://models` is a DEPRECATED alias of `claude-in-codex://models` kept for a compatibility
window; read the canonical `claude-in-codex://` URI in new work.
