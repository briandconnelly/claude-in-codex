...
- **Branch a launch on `outcome`, never on whether `job_id` is present.** Every `ok:true` reply from the three starters carries one of three values, and the machine-readable routing table is `claude_capabilities.async_lifecycle.start_outcome_routing`:
  ...
  - `existing_job` — an `idempotency_key` replay; this is a handle to a job that already existed, so it **may already be finished**. Read `may_be_terminal` and check `claude_job_status` before assuming there is anything left to wait for.
