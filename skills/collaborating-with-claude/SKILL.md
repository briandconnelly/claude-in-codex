...
- `claude_consult_async`, `claude_review_changes_async`, `claude_adversarial_review_async` — the same three calls as background jobs. These three are the only async forms; every blocking paid call has one, and the starters themselves are not nested further (there is no `claude_consult_async_async`). Use `claude_job_consume_result` only when you want to fetch and delete the stored record; use `claude_job_cancel` to stop a run early.
- **Branch a launch on `outcome`, never on whether `job_id` is present.** Every `ok:true` reply from the three starters carries one of three values, and the machine-readable routing table is `claude_capabilities.async_lifecycle.start_outcome_routing`:
  - `started` — a new paid job is running. Poll `claude_job_status`, then fetch with `claude_job_result`.
  - `existing_job` — an `idempotency_key` replay; this is a handle to a job that already existed, so it **may already be finished**. The reply is a job status: read its `status` and `result_available` and fetch with `claude_job_result` if it is already done, rather than polling something terminal.
  - `no_changes` — the diff was empty, so the spend was skipped and the result is inline. There is no job to poll; read the payload. Only the two diff-bearing starters can return this — `claude_consult_async` has no diff.

  `job_id`-presence cannot distinguish these: `started` and `existing_job` both carry one, and only `outcome` separates a fresh launch from a replay that may be terminal.
- `claude_status` — free readiness check: reports whether `claude` is installed, authenticated (`claude_authenticated`), version-compatible (`version_supported`), and overall `ready`, plus the resolved defaults a no-arg call would use. Run it before the first paid call of a session, and again whenever a call fails with a setup error.
...
- Never re-issue a paid call just because it failed. `retryable` is the server's answer to "will the identical call work later", and on a timeout it is `false`. A timeout cannot tell you whether Anthropic charged the call — the deadline also covers startup and workspace hooks, and no cost record survives — so treat the charge as possible and unrecoverable either way. Any next attempt is a NEW paid run, so decide deliberately to spend again rather than retrying on reflex. When you do, the `action` points at the `_async` twin, which survives the deadline; the `idempotency_key` it carries guards THAT new launch against duplicate retries, not against the call that timed out.
...
- Free-form `prompt`/`context`/`target`/`evidence`/`focus` text is capped before spend; split very large asks or use a narrower diff scope. `focus` and `system_prompt_append` also carry their own 4096-byte per-field caps. The diff selectors are capped too, so the response cannot grow without bound with your request: `paths` takes at most 256 entries of at most 4096 bytes each and 32768 bytes in total, and `base`/`head` at most 4096 bytes each. Over-cap values are refused with `invalid_paths`/`invalid_base`/`invalid_head` carrying the cap and your value's size, and the ref caps apply on every scope -- including one that ignores the ref.
