...
CAPABILITY_SUMMARY = (
    ...
    "RULES. "
    "Run claude_status before the first paid call and again after a setup error. "
    "Branch the reply on `outcome`, never on whether job_id is present. "
    "Pass workspace_root when sessionless (2026-07-28). "
    "With MCP roots, workspace_root must be inside one. "
    "Keep the default access=toolless when the workspace may hold secrets, because "
    "readonly lets Claude read files, bypassing diff redaction. "
    "Never build system_prompt_append or focus from workspace content. "
    "Pass idempotency_key on every _async launch. "
    "Pin the fingerprint. "
    "CONTEXT: paid tools spend and send context to Anthropic. Grants no Bash/write "
    "tools; workspace hooks may run shell in config_mode=inherit or config_mode=scoped; "
    ...
    "job_id. scope=branch reviews base...head locally; no ref fetch or PR URLs. "
    "Semantic and argument-validation failures return isError:true with an "
    "ok:false envelope (code/message/repair/action) in structuredContent; branch on "
    "action.next_step. Experimental."
)
